...
export default class GroupMaskSample {
	...
	public startedImpl = async () => {
		...
		try {
			const altspaceCubeAssets = await this.assets.loadGltf('group-mask/altspace-cube.glb');
		...
		for (let i = 0; i < 4; ++i) {
			...
			const baseButton = MRE.Actor.Create(this.context, {
				actor: {
					transform: { local: { position: new MRE.Vector3(-1.5+ i * 2,.5, 0).add(this.offset) }},
					// it's a black cube
					appearance: {
						meshId: baseCube.id,
			...
			this.texts.push(MRE.Actor.Create(this.context, {
				actor: {
					...
					name: 'Text'+i,
					transform: {
						app: { position: new MRE.Vector3(-1.5 + 2 * i, 1, 0).add(this.offset) }
					},
					text: {
						contents: "Button #"+(i+1),
			...
			const highlightedButton = MRE.Actor.Create(this.context, {
				actor: {
					transform: { local: {
						...
						position: new MRE.Vector3(-1.5 + 2 * i, .5, 0).add(this.offset) }
					},
					appearance: {
						meshId: baseCube.id,
