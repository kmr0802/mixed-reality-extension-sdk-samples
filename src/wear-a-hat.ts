...
const HatDatabase: HatDatabase = require('../public/wear-a-hat/hats.json');
...
export default class WearAHat {
	...
	private showHatMenu() {
		...
		for (const hatId of Object.keys(HatDatabase)) {
			...
			const button = MRE.Actor.Create(this.context, {
				actor: {
					...
					transform: {
						local: { position: new MRE.Vector3(0, y, 0).add(this.offset) }
					}
				}
			});
			...
			MRE.Actor.Create(this.context, {
				actor: {
					...
					transform: {
						local: { position: new MRE.Vector3(0.5, y, 0).add(this.offset) }
					}
				}
			});
			y = y + 0.5;
		...
		MRE.Actor.Create(this.context, {
			actor: {
				...
				transform: {
					local: { position: new MRE.Vector3(0.5, y + 0.25, 0).add(this.offset) }
				}
			}
		});
	}
		...
		return Promise.all(
			Object.keys(HatDatabase).map(hatId => {
				...
				if (hatRecord.resourceName) {
					return this.assets.loadGltf('wear-a-hat/' + hatRecord.resourceName)
						.then(assets => {
	...
	private wearHat(hatId: string, userId: MRE.Guid) {
		...
		MRE.log.debug('app', 'Wearing ' + hatRecord.resourceName);
		...
		this.attachedHats.set(userId, MRE.Actor.CreateFromPrefab(this.context, {
			...
			actor: {
				transform: {
					local: {
						position: new MRE.Vector3(
							hatRecord.position.x,
							hatRecord.position.y,
							hatRecord.position.z
						),
