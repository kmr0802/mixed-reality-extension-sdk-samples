	...
	public startedImpl = async () => {
		...
		this.text = MRE.Actor.Create(this.context, {
			actor: {
				name: 'Text',
				transform: {
					app: { position: new MRE.Vector3(0, 0.5, 0).add(this.offset) }
				},
				text: {
		...
		const cubeData = await this.assets.loadGltf('hello-world/altspace-cube.glb', "box");
		...
		this.cube = MRE.Actor.CreateFromPrefab(this.context, {
			...
			actor: {
				...
				transform: {
					local: {
						position: new MRE.Vector3(0, -1, 0).add(this.offset),
