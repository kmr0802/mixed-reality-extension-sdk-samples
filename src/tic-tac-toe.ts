	...
	public startedImpl = async () => {
		...
		this.textAnchor = MRE.Actor.Create(this.context, {
			actor: {
				name: 'TextAnchor',
				transform: {
					app: { position: new MRE.Vector3(0, 1.2, 0).add(this.offset) }
				},
			}
		});
		...
		this.text = MRE.Actor.Create(this.context, {
			actor: {
				...
				name: 'Text',
				transform: {
					local: { position: new MRE.Vector3(0, 0, -1.5).add(this.offset) }
				},
				text: {
		...
		this.light = MRE.Actor.Create(this.context, {
			actor: {
				...
				transform: {
					local: {
						position: new MRE.Vector3(0, 1, -0.5).add(this.offset),
		...
		const gltf = await this.assets.loadGltf('tic-tac-toe/altspace-cube.glb', 'box');
			...
			for (let tileIndexZ = 0; tileIndexZ < 3; tileIndexZ++) {
				...
				const cube = MRE.Actor.CreateFromPrefab(this.context, {
					...
					actor: {
						...
						transform: {
							app: {
								position: new MRE.Vector3((tileIndexX) - 1.0, 0.5, (tileIndexZ) - 1.0).add(this.offset),
							},
						...
						case GameState.Play:
							...
							if (this.boardState[tileIndexX * 3 + tileIndexZ] === undefined) {
								...
								const gamePiecePosition = new MRE.Vector3(
									cube.transform.app.position.x,
									cube.transform.app.position.y + 0.55,
									cube.transform.app.position.z);
